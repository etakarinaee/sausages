...
// `input_*_just_*` api returns `true` only on the single frame where the state
// transitions, whereas the standard api returns `true` on every frame while the
// key is held down
...
void input_mouse_position(const struct input_context *ctx, double *x,
                          double *y);
