...
struct vec2i math_vec2_to_vec2i(struct vec2 v) {
    return (struct vec2i){(int)v.x, (int)v.y};
}
...
void math_matrix_identity(struct matrix *m) {
    *m = (struct matrix){
        .m = {[0] = 1.0f, [5] = 1.0f, [10] = 1.0f, [15] = 1.0f}};
}

void math_matrix_translate(struct matrix *m, const float x, const float y,
                           const float z) {
...
void math_matrix_scale(struct matrix *m, const float x, const float y,
                       const float z) {
    *m = (struct matrix){.m = {[0] = x, [5] = y, [10] = z, [15] = 1.0f}};
}
